<!-- Custom CSS -->
<link href="<?php echo e(asset('assets/extra-libs/horizontal-timeline/horizontal-timeline.css')); ?>" rel="stylesheet">
<link href="<?php echo e(asset('dist/css/style.min.css')); ?>" rel="stylesheet">
<!-- This page plugin CSS -->
<link rel="stylesheet" type="text/css" href="<?php echo e(asset('assets/extra-libs/css-chart/css-chart.css')); ?>">
<link href="<?php echo e(asset('assets/libs/datatables.net-bs4/css/dataTables.bootstrap4.css')); ?>" rel="stylesheet">
<link href="<?php echo e(asset('assets/libs/jquery-steps/jquery.steps.css')); ?>" rel="stylesheet">
<link href="<?php echo e(asset('assets/libs/jquery-steps/steps.css')); ?>" rel="stylesheet">
<link rel="stylesheet" type="text/css" href="<?php echo e(asset('assets/extra-libs/prism/prism.css')); ?>">
<link rel="stylesheet" type="text/css" href="<?php echo e(asset('assets/libs/dragula/dist/dragula.min.css')); ?>">
<script src="<?php echo e(asset('assets/libs/dragula/dist/dragula.min.js')); ?>"></script>
<link href="<?php echo e(asset('assets/libs/magnific-popup/dist/magnific-popup.css')); ?>" rel="stylesheet">
<link href="<?php echo e(asset('assets/libs/magnific-popup/animation.css')); ?>" rel="stylesheet">
<!-- Custom CSS -->
<link href="<?php echo e(asset('assets/libs/chartist/dist/chartist.min.css')); ?>" rel="stylesheet">
<link href="<?php echo e(asset('assets/extra-libs/c3/c3.min.css')); ?>" rel="stylesheet">
<link href="<?php echo e(asset('assets/extra-libs/jvector/jquery-jvectormap-2.0.2.css')); ?>" rel="stylesheet" />
<!-- chatbot CSS -->
<link href="<?php echo e(asset('assets/css/style_chat.css')); ?>" rel="stylesheet">
<!-- font Ar -->
<link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700&display=swap" rel="stylesheet">
<?php /**PATH D:\Projects\مشروع الكلية\project code\f.s.s.r\resources\views/layout/styles.blade.php ENDPATH**/ ?>